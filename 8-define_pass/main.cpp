...
mlir::ModuleOp getModule(mlir::OpBuilder& builder) {
    ...
    auto dy_tensor_type = mlir::dream_star::DSTensorType::get(context, dy_shape, f32, 0);
    auto func_type = mlir::FunctionType::get(context,{dy_tensor_type},{dy_tensor_type});
    auto func = builder.create<mlir::func::FuncOp>(loc, KEntryPointName, func_type);
    func->setAttr(KHostFunc, builder.getUnitAttr());
    func->setAttr(KDPAttrName, mlir::dream_star::DataParallelismAttr::get(context, 2));

    auto block = func.addEntryBlock();
    builder.setInsertionPointToStart(block);

    //softmax op
    mlir::Value softmax_op = builder.create<mlir::dream_star::SoftmaxOp>(loc, block->getArgument(0), 1);
