...
static inline llvm::SmallString<4> getFusionName(
  ...
  for (auto op : ops) {
    name.append(op->getName().stripDialect());  // stripDialect去掉前缀
    name.append("_");
...
static inline int getDeviceId(mlir::ArrayRef<::mlir::Operation*> ops) {
  if (auto tensor = llvm::cast_or_null<dream_star::DSTensorType>(
...
void FusionOps(::mlir::RewriterBase& rewriter,
  ...
  llvm::SmallVector<Value> outputs_val;
  ...
  for (auto [key, val] : outputs_map) {
    outputs_val.push_back(op_map[val.first]->getResult(val.second));
  }
  ...
  rewriter.create<func::ReturnOp>(loc, outputs_val);
...
struct BufferCastOpDeviceRegionFusion
    : public OpRewritePattern<::mlir::dream_star::BufferCastOp> {
  using OpRewritePattern::OpRewritePattern;

  virtual LogicalResult matchAndRewrite(::mlir::dream_star::BufferCastOp op,
...
struct BufferCastOpFold
  ...
  virtual LogicalResult match(::mlir::dream_star::BufferCastOp op) const {
    llvm::outs() << "match:" << getDebugName() << "\n";
  ...
  virtual void rewrite(::mlir::dream_star::BufferCastOp op,
                       PatternRewriter& rewriter) const {
...
void ::mlir::dream_star::populateDeviceRegionFusionPatterns(
...
void ::mlir::dream_star::populateBufferCastOpCanonicalizationPatterns(
    RewritePatternSet& patterns) {
  auto context = patterns.getContext();
...
struct DeviceRegionFusionPass
    : ::mlir::dream_star::impl::DeviceRegionFusionPassBase<
...
void DeviceRegionFusionPass::runOnOperation() {
  ...
  ::mlir::dream_star::populateBufferCastOpCanonicalizationPatterns(
  ...
  ::mlir::dream_star::populateDeviceRegionFusionPatterns(patterns);
