...
::llvm::LogicalResult BufferCastOp::verify() {
  if (getNumResults() > 1) {
    if (!llvm::all_of(getResultTypes(),
                      [](Type type) { return isa<DSTensorType>(type); }))
      return llvm::failure();
  }
  if (getNumOperands() > 1) {
...
llvm::SmallVector<Type> splitTensor(const DSTensorType& tensor, int dim,
...
::llvm::LogicalResult SoftmaxOp::applyDataParallelism(
  ...
  for (auto [index, operand] : llvm::enumerate(operands)) {
    auto type = llvm::dyn_cast_or_null<DSTensorType>(operand.getType());
    auto types = splitTensor(type, 0, device_ids);
    auto cast = builder.create<dream_star::BufferCastOp>(
  ...
  for (auto [index, res] : llvm::enumerate(results)) {
    auto type = llvm::dyn_cast_or_null<DSTensorType>(res.getType());
    auto types = splitTensor(type, 0, device_ids);
    for (auto [op_index, sub_op] : llvm::enumerate(ops)) {
