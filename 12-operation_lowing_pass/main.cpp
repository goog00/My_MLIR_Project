...
mlir::ModuleOp getModule(mlir::OpBuilder& builder) {
  ...
  auto dy_tensor_type =
      mlir::dream_star::DSTensorType::get(context, dy_shape, f32, 0);
  auto func_type =
  ...
  func->setAttr(KDPAttrName,
                mlir::dream_star::DataParallelismAttr::get(context, 2));

  auto block = func.addEntryBlock();
  builder.setInsertionPointToStart(block);

  // softmax op
  mlir::Value softmax_op = builder.create<mlir::dream_star::SoftmaxOp>(
      loc, block->getArgument(0), 1);
  softmax_op = builder.create<mlir::dream_star::SoftmaxOp>(loc, softmax_op, 1);
...
void CH8() {
  ...
  mlir::dream_star::MarkDistributeParallelParametersPassOptions
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::dream_star::createApplyDistributeTransformPass());
...
void CH9() {
  ...
  mlir::dream_star::MarkDistributeParallelParametersPassOptions
      mark_distribute_parallel_option{.DPNums = 3, .TPNums = 1};
 //c++17     
//   MarkDistributeParallelParametersPassOptions mark_distribute_parallel_option;
//   mark_distribute_parallel_option.DPNums = 3;
//   mark_distribute_parallel_option.TPNums = 1;
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::dream_star::createApplyDistributeTransformPass());
  pm.addNestedPass<mlir::func::FuncOp>(
      mlir::dream_star::createDeviceRegionFusionPass());
  module->dump();
  if (pm.run(module).failed()) {
    llvm::outs() << "run pass error!\n";
  };
